...
def run_simulation(length: float , width: float, force1: float, force2: float, force3: float, force4: float, young_modulus: str, poisson_ratio: str) -> float:
    ...
    if (force1 == int(force1)):
        force1 = int(force1)

    if (force2 == int(force2)):
        force2 = int(force2)

    if (force3 == int(force3)):
        force3 = int(force3)
        
    if (force4 == int(force4)):
        force4 = int(force4)

    App = FreeCAD
    doc = App.newDocument("temp")
    ...
    fea.purge_results()
    fea.run()
    results = App.getDocument("temp").getObject("CCX_Results")
    res_length = max(results.DisplacementLengths)
    App.Console.PrintMessage("Value  : " + str(res_length)) 


run_simulation(float(sys.argv[2]), float(sys.argv[3]), float(sys.argv[4]), float(sys.argv[5]), float(sys.argv[6]), float(sys.argv[7]), sys.argv[8] + " " + sys.argv[9], sys.argv[10])
