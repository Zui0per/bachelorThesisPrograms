...
def run_simulation(id: int, length: float , width: float, force1: float, force2: float, force3: float, force4: float, young_modulus: str, poisson_ratio: str) -> float:

    App = FreeCAD
    doc = App.newDocument(f"temp_{id}")
    App.setActiveDocument(doc.Label)
    box_obj = doc.addObject('Part::Box', 'Box')
    box_obj.Height = box_obj.Width = width
    box_obj.Length = length

    # Remove to run in cmd mode
    ...
    # force_constraint1
    force_constraint = ObjectsFem.makeConstraintForce(doc, "FemConstraintForce1")
    force_constraint.References = [(doc.Box, "Face2")]
    force_constraint.Force = force1
    force_constraint.Direction = (doc.Box, ["Edge5"])
    force_constraint.Reversed = True
    analysis_object.addObject(force_constraint)

    # force_constraint2
    force_constraint = ObjectsFem.makeConstraintForce(doc, "FemConstraintForce2")
    force_constraint.References = [(doc.Box, "Edge7")]
    force_constraint.Force = force2
    force_constraint.Direction = (doc.Box, ["Edge8"])
    force_constraint.Reversed = True
    analysis_object.addObject(force_constraint)

    # force_constraint3
    force_constraint = ObjectsFem.makeConstraintForce(doc, "FemConstraintForce3")
    force_constraint.References = [(doc.Box, "Face3")]
    force_constraint.Force = force3
    force_constraint.Direction = (doc.Box, ["Edge8"])
    analysis_object.addObject(force_constraint)

    # force_constraint4
    force_constraint = ObjectsFem.makeConstraintForce(doc, "FemConstraintForce4")
    force_constraint.References = [(doc.Box, "Edge12")]
    force_constraint.Force = force4
    force_constraint.Direction = (doc.Box, ["Edge8"])
    force_constraint.Reversed = True
    analysis_object.addObject(force_constraint)
    ...
    App.getDocument(doc.Label).saveAs(f"C:/Users/oskar/Desktop/freecadPython/simulations/sim_{id}.FCStd")
    results = App.getDocument(f"temp_{id}").getObject("CCX_Results")
    res_length = max(results.DisplacementLengths)
    App.Console.PrintMessage("Value  : " + str(res_length)) 

    FreeCADGui.getMainWindow().deleteLater()


run_simulation(int(sys.argv[2]), float(sys.argv[3]), float(sys.argv[4]), float(sys.argv[5]), float(sys.argv[6]), float(sys.argv[7]), float(sys.argv[8]), sys.argv[9] + " " + sys.argv[10], sys.argv[11])
